...
// RXB: Global connection manager for RPC commands
 ...
 */
static const char* RXB_PID_FILENAME = "rxbd.pid";

static fs::path GetPidFile()
{
    return AbsPathForConfigVal(fs::path(gArgs.GetArg("-pid", RXB_PID_FILENAME)));
}
...
void Shutdown(NodeContext& node)
{
    ...
    // RXB: Stop and clean up miner before destroying connman
    if (g_miner) {
        g_miner->StopMining();
        g_miner.reset();
    }
...
void SetupServerArgs()
{
    ...
    gArgs.AddArg("-conf=<file>", strprintf("Specify configuration file. Relative paths will be prefixed by datadir location. (default: %s)", RXB_CONF_FILENAME), ArgsManager::ALLOW_ANY, OptionsCategory::OPTIONS);
    ...
    gArgs.AddArg("-pid=<file>", strprintf("Specify pid file. Relative paths will be prefixed by a net-specific datadir location. (default: %s)", RXB_PID_FILENAME), ArgsManager::ALLOW_ANY, OptionsCategory::OPTIONS);
    ...
    gArgs.AddArg("-acceptnonstdtxn", strprintf("Relay and mine \"non-standard\" transactions (default: %u)", false), ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::NODE_RELAY);
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/Heiwabitnull/rxb-core>";
...
/** Sanity checks
 *  Ensure that RXB is running in a usable environment with all
...
bool AppInitParameterInteraction()
{
    ...
    if (gArgs.GetArg("-mocktime", 0) != 0) return InitError("-mocktime is not supported.");
...
{
    // Make sure only a single RXB process is using the data directory.
...
bool AppInitMain(NodeContext& node)
{
    ...
    fs::path config_file_path = GetConfigFile(gArgs.GetArg("-conf", RXB_CONF_FILENAME));
    ...
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the " /* Continued */
                  "current working directory '%s'. This is fragile, because if rxb is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if rxb is started while in a temporary directory.\n",
    ...
    node.scheduler = std::unique_ptr<CScheduler>(new CScheduler());
    ...
    assert(!node.banman);
    node.banman = std::unique_ptr<BanMan>(new BanMan(GetDataDir() / "banlist.dat", &uiInterface, gArgs.GetArg("-bantime", DEFAULT_MISBEHAVING_BANTIME)));
    assert(!node.connman);
    ...
    // RXB: Set global connection manager for RPC
    g_connman = node.connman.get();

    // RXB: Initialize global miner instance (C++11 compatible)
    g_miner = std::unique_ptr<RXBMiner>(new RXBMiner(node.connman.get()));
    ...
    while (!fLoaded && !ShutdownRequested()) {
        ...
        do {
            ...
            try {
                ...
                g_chainstate = std::unique_ptr<CChainState>(new CChainState());
                UnloadBlockIndex();
    ...
    if (gArgs.GetBoolArg("-txindex", DEFAULT_TXINDEX)) {
        g_txindex = std::unique_ptr<TxIndex>(new TxIndex(nTxIndexCache, false, fReindex));
        g_txindex->Start();
    }
