...
# RXB Build Script for Guix Environment
...
# Setup the directory where our RXB build for HOST will occur
...
make -C depends --jobs="$MAX_JOBS" HOST="$HOST" \
                                   ...
                                   qt_config_opts_i686_linux='-platform linux-g++ -xplatform rxb-linux-g++'

###########################
# Source Tarball Building #
###########################

echo "Creating source tarball for RXB..."

# Create the source tarball and move it to "${OUTDIR}/src" if not already there
if [ -z "$(find "${OUTDIR}/src" -name 'rxb-*.tar.gz')" ]; then
    ./autogen.sh
    env CONFIG_SITE="${BASEPREFIX}/${HOST}/share/config.site" ./configure --prefix=/
    make dist GZIP_ENV='-9n' ${V:+V=1}
    mkdir -p "${OUTDIR}/src"
    mv "$(find "${PWD}" -name 'rxb-*.tar.gz')" "${OUTDIR}/src/"
    echo "Source tarball created: $(find "${OUTDIR}/src" -name 'rxb-*.tar.gz')"
fi
...
echo "Building RXB binaries for ${HOST}..."
...
(
    ...
    echo "Extracting source tarball: ${SOURCEDIST}"
    tar --strip-components=1 -xf "${SOURCEDIST}"

    # Configure this DISTSRC for $HOST
    echo "Configuring RXB for ${HOST}..."
    ...
    # Build RXB
    echo "Building RXB (this may take a while)..."
    ...
    # Setup the directory where our RXB build for HOST will be
    # installed. This directory will also later serve as the input for our
    # binary tarballs.
    INSTALLPATH="${PWD}/installed/${DISTNAME}"
    mkdir -p "${INSTALLPATH}"
    
    # Install built RXB to $INSTALLPATH
    echo "Installing RXB to ${INSTALLPATH}..."
...
echo "Binary tarballs created in ${OUTDIR}:"
ls -la "${OUTDIR}/${DISTNAME}-${HOST}.tar.gz" "${OUTDIR}/${DISTNAME}-${HOST}-debug.tar.gz" 2>/dev/null || true
echo ""
echo "To verify the build:"
echo "  tar -tzf ${OUTDIR}/${DISTNAME}-${HOST}.tar.gz | head -20"
echo ""
